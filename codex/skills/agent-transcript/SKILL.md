...
Best-effort local-only provenance for agent-authored PR/issue bodies. Use during agent-created GitHub PR or issue workflows before creating or updating the body.
...
The Codex setup script installs this skill to `~/.agents/skills/agent-transcript/` by default. If `CODEX_SKILLS_DIR` was overridden during install, replace `~/.agents/skills` in these commands with that directory.
...
Vendored from [steipete/agent-scripts](https://github.com/steipete/agent-scripts) (`skills/agent-transcript`, MIT), commit `2d007c16`. The helper under `scripts/` is a byte-for-byte copy of upstream; only this SKILL.md's command paths were adapted for the harness-configs Codex install location.

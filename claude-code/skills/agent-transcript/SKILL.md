...
allowed-tools: Bash(~/.claude/skills/agent-transcript/scripts/agent-transcript:*)
---
...
```bash
~/.claude/skills/agent-transcript/scripts/agent-transcript --help
```

Find a likely local session:

```bash
~/.claude/skills/agent-transcript/scripts/agent-transcript find \
...
The paths above are absolute, so the commands work from any cwd. `setup.sh` installs this skill to `~/.claude/skills/agent-transcript/`.

Render a PR/issue body section:

```bash
~/.claude/skills/agent-transcript/scripts/agent-transcript render \
...
```bash
~/.claude/skills/agent-transcript/scripts/agent-transcript preview \
  --session "$SESSION_JSONL" \
...
```bash
~/.claude/skills/agent-transcript/scripts/agent-transcript append-body \
  --body /tmp/pr-body.md \
  --session "$SESSION_JSONL" \
...
```bash
~/.claude/skills/agent-transcript/scripts/agent-transcript html \
...
Vendored from [steipete/agent-scripts](https://github.com/steipete/agent-scripts) (`skills/agent-transcript`, MIT), commit `2d007c16`. The helper under `scripts/` is a byte-for-byte copy of upstream; only this SKILL.md's command paths were adapted for the harness-configs install location.
